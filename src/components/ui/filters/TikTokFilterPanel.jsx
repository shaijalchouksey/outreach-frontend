...
export default function TikTokFilterPanel({ onClose, onApply }) {
  ...
  const [keywords, setKeywords] = useState(["#TikTokTrend", "#TikTokReels"]);
    ...
    >
      ...
      <div className="p-4 sm:p-6 space-y-6 overflow-y-auto flex-1 scrollbar-thin scrollbar-thumb-gray-300 scrollbar-track-gray-100">
        ...
        <div>
          ...
          {!isCollapsed("Source Type") && (
            <div className="space-y-2">
              {["TikTok Reels", "TikTok Ads Library"].map((type) => (
                <label key={type} className="flex items-center gap-2 text-sm">
                  <input
                    type="checkbox"
        ...
        <div>
          <button
            onClick={() => toggleCollapse("Advance Options")}
            className="flex justify-between items-center w-full mb-3"
          >
            <h4 className="font-medium text-gray-800">Advance Options</h4>
            {isCollapsed("Advance Options") ? (
              <ChevronDown className="w-4 h-4 text-gray-400" />
            ) : (
              <ChevronUp className="w-4 h-4 text-gray-400" />
            )}
          ...
          {!isCollapsed("Advance Options") && (
            <div className="space-y-2">
              {["Highest Engagement", "Most Recent"].map((opt) => (
                <label key={opt} className="flex items-center gap-2 text-sm">
                  <input
                    type="checkbox"
                    checked={advancedOptions.includes(opt)}
                    onChange={() =>
                      toggleArray(advancedOptions, setAdvancedOptions, opt)
                    }
                  />
                  {opt}
                </label>
